...
Deploy multi-turn reasoning loops, autonomous coding workflows, web intelligence agents, and tool-augmented workflows powered by Claude Opus 4.6, DeepSeek v3.2, and Gemini.

The **Agent Compute Engine** orchestrates dynamic tool calling, skill execution, persistent workspace file storage, real-time Server-Sent Events (SSE) streaming, and Human-in-the-Loop controls.

:::info API Environment
- **Base URL**: `https://comp1.fotohub.app/v1`
- **Region**: `eu-central-1` (Frankfurt, AWS)
- **Billing**: USD prepaid wallet first, subscription credits as fallback. A task will not start with $0.00 wallet balance and fewer than 5 credits.
...
3. **Sandbox Dispatch**: If a tool requires execution (like Python code), the orchestrator securely dispatches it internally to its Firecracker microVM backend (proxy-secret authenticated, not a public endpoint — see [Firecracker Sandboxes](/compute/agent-sandboxes)).
...
:::danger Wallet First, Credits as Fallback
Agent tasks bill the prepaid USD wallet first and fall back to subscription credits. A task is
rejected before it starts if your wallet is at $0.00 **and** you hold fewer than 5 subscription
credits.
:::

### Language Models

Pricing below matches the live per-model rate table used by the Agent Compute service.

| Model | Provider | Best For | Price (Input / 1M) | Price (Output / 1M) |
|---|---|---|---|---|
| **Claude Opus 4.6** | Anthropic | Complex reasoning, large refactors, autonomous software engineering | $15.00 | $75.00 |
| **Claude Sonnet 4.6** | Anthropic | Fast coding, tool use, web automation (fallback model) | $3.00 | $15.00 |
| **Claude Haiku 4.5** | Anthropic | Low-latency, high-volume tool steps | $1.00 | $5.00 |
| **Gemini 3.1 Pro (preview)** | Google | Multimodal reasoning, long-context document analysis | $2.00 | $12.00 |
| **Gemini 2.5 Flash** | Google | High-volume, latency-sensitive tasks | $0.30 | $2.50 |
| **Grok 4.20** | xAI | Reasoning and non-reasoning agentic steps | $2.00 | $6.00 |
| **DeepSeek v3.2** | DeepSeek | Mathematical reasoning, optimization | $0.50 | $2.00 |
| **Kimi K2** | Moonshot AI | Long-horizon planning, thinking mode | $0.60 | $2.40 |
| **Qwen3 Max** | Alibaba | General purpose reasoning | $1.00 | $4.00 |
| **GPT-5.1** | Azure OpenAI | General purpose, reliable tool calling | $3.00 | $15.00 |

The default model is **Claude Opus 4.6**, with **Claude Sonnet 4.6** as the automatic fallback.
...
Agents can execute code in a secure Firecracker sandbox — dispatched internally, not via a public URL. See [Firecracker Sandboxes](/compute/agent-sandboxes) for the mechanism.
...
- **Limitations**: Workspace storage is not separately metered or billed today — only model tokens and sandbox wall-clock time show up on your invoice for a task.
...
# Agent Compute is not covered by the fotohub SDK -- plain HTTP.
BASE = "https://comp1.fotohub.app/v1"
HEADERS = {"Authorization": f"Bearer {os.environ['FOTOHUB_API_KEY']}"}

task = requests.post(f"{BASE}/tasks/create", headers=HEADERS, timeout=60, json={
    "prompt": """
...
const BASE = "https://comp1.fotohub.app/v1";
const HEADERS = {
...
async function dispatchAgent() {
  ...
  console.log("Agent running with Task ID:", task.task_id);
}
...
func main() {
	...
	url := "https://comp1.fotohub.app/v1/tasks/create"
...
```bash [cURL]
curl -X POST https://comp1.fotohub.app/v1/tasks/create   -H "Authorization: Bearer $FOTOHUB_API_KEY"   -H "Content-Type: application/json"   -d '{
...
const eventSource = new EventSource(
  `https://comp1.fotohub.app/v1/tasks/${taskId}/stream?token=${FOTOHUB_API_KEY}`
);
...
```bash
curl -X POST https://comp1.fotohub.app/v1/tasks/tsk_99a812df/pause   -H "Authorization: Bearer $FOTOHUB_API_KEY"
```
...
```bash
curl -X POST https://comp1.fotohub.app/v1/tasks/tsk_99a812df/respond   -H "Authorization: Bearer $FOTOHUB_API_KEY"   -H "Content-Type: application/json"   -d '{
    "approved": true,
...
```bash
curl -X POST https://comp1.fotohub.app/v1/tasks/tsk_99a812df/cancel   -H "Authorization: Bearer $FOTOHUB_API_KEY"
```
...
- **Multi-Agent Swarm Consensus**: Aggregate decisions from multiple autonomous agents via `/engine/v1/swarm/consensus`.
- **Vector Memory RAG**: Store and retrieve long-term state across sessions via `/engine/v1/memory/*`.
- **Canary Deployments**: Roll out agent logic changes safely with traffic routing via `/engine/v1/workflows/{id}/canary`.
- **Dead-Letter Queue (DLQ)**: Automatically capture failures and replay them via `/engine/v1/incidents`.
- **Zero-Secret OAuth Hub**: Seamlessly connect agents with 13 external SaaS services (Slack, Google, Meta, Shopify, Notion, TikTok, LinkedIn, X/Twitter, Airtable, Stripe, Microsoft, Dropbox, Discord).

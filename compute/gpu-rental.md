...
instance_res = requests.post(f"{BASE}/compute/v1/instances", headers=HEADERS, timeout=60, json={
...
const res = await fetch("https://apis.fotohub.app/compute/v1/instances", {
  ...
  body: JSON.stringify({
    ...
    catalog_id: "g4dn.xlarge",       // NVIDIA T4, $0.1975/hr spot
    spot_instance: true,
...
instances = []
for zone in zones:
    res = requests.post(f"{BASE}/compute/v1/instances", headers=HEADERS, timeout=60, json={
        "name": f"worker-{zone}",
...
1. **Tensor Parallelism:** Not applicable — every FOTOhub GPU instance carries a single GPU (one T4 or one A10G), so there is no multi-GPU tier to shard a model across. If a model doesn't fit in 24 GB, quantize it instead.

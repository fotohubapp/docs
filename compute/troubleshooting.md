...
:::code-group
```bash [cURL]
curl -X POST "https://apis.fotohub.app/billing/v1/wallet/top-up" \
  -H "Authorization: Bearer $FOTOHUB_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"amount_usd": 50.00, "payment_method_id": "pm_12345"}'
```
```python [Python]
import requests
headers = {"Authorization": f"Bearer {API_KEY}"}
payload = {"amount_usd": 50.00, "payment_method_id": "pm_12345"}
requests.post("https://apis.fotohub.app/billing/v1/wallet/top-up", json=payload, headers=headers)
```
```typescript [TypeScript]
import axios from 'axios';
await axios.post('https://apis.fotohub.app/billing/v1/wallet/top-up', {
  amount_usd: 50.00, payment_method_id: "pm_12345"
}, { headers: { Authorization: `Bearer ${API_KEY}` } });
```
```go [Go]
import "bytes"
import "net/http"
payload := []byte(`{"amount_usd": 50.00, "payment_method_id": "pm_12345"}`)
req, _ := http.NewRequest("POST", "https://apis.fotohub.app/billing/v1/wallet/top-up", bytes.NewBuffer(payload))
req.Header.Set("Authorization", "Bearer "+API_KEY)
req.Header.Set("Content-Type", "application/json")
client := &http.Client{}
resp, _ := client.Do(req)
...
### 13. Edge Case: Handling Stale NFS Mounts
...
### 14. Edge Case: Handling Stale NFS Mounts
...
### 15. Edge Case: Handling Stale NFS Mounts
...
### 16. Edge Case: Handling Stale NFS Mounts
...
### 17. Edge Case: Handling Stale NFS Mounts
...
### 18. Edge Case: Handling Stale NFS Mounts
...
### 19. Edge Case: Handling Stale NFS Mounts
...
### 20. Edge Case: Handling Stale NFS Mounts
...
### 21. Edge Case: Handling Stale NFS Mounts
...
### 22. Edge Case: Handling Stale NFS Mounts
...
### 23. Edge Case: Handling Stale NFS Mounts
...
### 24. Edge Case: Handling Stale NFS Mounts
...
### 25. Edge Case: Handling Stale NFS Mounts
...
### 26. Edge Case: Handling Stale NFS Mounts
...
### 27. Edge Case: Handling Stale NFS Mounts
...
### 28. Edge Case: Handling Stale NFS Mounts
...
### 29. Edge Case: Handling Stale NFS Mounts
...
### 30. Edge Case: Handling Stale NFS Mounts
...
### 31. Edge Case: Handling Stale NFS Mounts
...
### 32. Edge Case: Handling Stale NFS Mounts

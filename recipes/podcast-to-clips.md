...
The complete Shorts Engine lifecycle requires coordinating these REST endpoints:

1. `POST /v1/shorts/clips` — Submit a long-form video for asynchronous clipping.
2. `GET /v1/shorts/clips/{job_id}` — Poll for status, live progress, and final outputs. There is no separate SSE stream for a single job — poll this endpoint.
3. `POST /v1/ai/transcribe` — Standalone endpoint for transcription.
...
This page previously documented a per-job `GET /v1/shorts/clips/{job_id}/stream`
Server-Sent Events endpoint. **It does not exist** — there is no SSE stream for an
individual shorts job anywhere in the API. Build your progress bar by polling
`GET /v1/shorts/clips/{job_id}` (section 4 above), which already returns
...
def poll_job_progress(job_id: str):
    url = f"https://apis.fotohub.app/v1/shorts/clips/{job_id}"
    headers = {"Authorization": "Bearer fh_live_your_api_key"}
...
## 6. Delivering clips to your own storage (BYOB)

::: warning Corrected 2026-09
This page previously documented a per-job `POST /v1/shorts/export` call that took
a `job_id` plus raw AWS/R2 credentials in the request body. **That endpoint does
not exist**, and sending long-lived secret keys in a per-call JSON body would be
a bad design if it did.

The real BYOB mechanism is account-wide, not per-job: register a destination
**once** with `POST /v1/destinations`, and everything that API key generates —
not just shorts clips — is mirrored there automatically. There is no
per-job "export this one clip" call; you either receive the clip URL directly
in the poll response, or attach a standing destination up front.
:::

### Registering a destination (one-time setup)
...
curl -X POST https://apis.fotohub.app/v1/destinations \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "name": "Production shorts bucket",
    "providerPreset": "aws",
    "bucketName": "my-production-shorts",
    "region": "us-west-2",
    "accessKeyId": "AKIA...",
...
For Cloudflare R2, use `"providerPreset": "r2"` and add `"accountId"`. See
[Cloud Storage](/api/storage) for the full destination reference.
